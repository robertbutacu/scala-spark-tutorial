...
object HousePriceProblem extends App{
  ...
  val session = SparkSession.builder().appName("HousePricingProblem").master("local[1]").getOrCreate()
  ...
  val housePricing = responses.select(LOCATION, PRICE_SQ_FT)

  housePricing.groupBy(LOCATION)
    .avg(PRICE_SQ_FT)
